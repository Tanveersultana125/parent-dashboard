...
async function adminFetchAll(ref, label) {
    const out = [];
    let cursor = null;
...
exports.aggregateSchoolStats = functions
    ...
    const [branchesDocs, studentsDocs, attendanceDocs, resultsDocs, testScoresDocs, feesDocs, teachersDocs, enrollmentsDocs] = await Promise.all([
    ...
    const branches = branchesDocs.map((d, i) => ({
        ...
        name: (d.data().name || d.data().schoolName || `Branch ${i + 1}`),
