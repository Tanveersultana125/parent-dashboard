...
exports.parentAIProxy = functions
    ...
    try {
        ...
        if (status === 401 || openaiCode === "invalid_api_key") {
            throw new functions.https.HttpsError("failed-precondition", "AI service is not configured correctly. Please contact support.");
        }
        if (status === 429 || openaiCode === "rate_limit_exceeded" || openaiCode === "insufficient_quota") {
            throw new functions.https.HttpsError("resource-exhausted", "AI service is at capacity right now — please try again in a minute.");
        }
        if (status === 408 || openaiCode === "ETIMEDOUT" || openaiCode === "ECONNRESET") {
            throw new functions.https.HttpsError("deadline-exceeded", "AI request timed out — please retry.");
        }
